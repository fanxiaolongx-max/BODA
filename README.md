...
- 💳 **在线支付** - 支持 Stripe 在线支付（Apple Pay、银行卡等）
- 💬 **反馈与投诉** - 快速反馈和投诉功能，自动发送邮件通知管理员
...
- ⚙️ **系统设置** - 点单开关、商店名称、货币符号等配置
  - **邮件配置** - SMTP邮件服务器配置，支持发送周期导出邮件和反馈通知
  - **安全策略配置** - 管理员和用户登录安全策略（锁定阈值、时间窗口、渐进延迟）
  - **IP管理** - IP锁定管理和解锁功能
...
- ✅ **渐进式账户锁定**（管理员和用户登录失败锁定，支持自定义策略）
- ✅ **IP级速率限制**（防止暴力破解攻击）
- ✅ **登录审计日志**（记录所有登录尝试，成功和失败）
...
- ✅ 友好的错误提示
- ✅ 实时数据更新
- ✅ 多语言支持（中英文切换，界面和错误提示完整翻译）
- ✅ 动态商店名称和货币符号
- ✅ 在线支付集成（Stripe Payment Element，支持 Apple Pay）
- ✅ 反馈与投诉系统（用户可直接提交反馈，自动邮件通知）
